...
function GuideCover() {
  return (
    <div className="relative">
      ...
      />
      <div className="relative bg-[#071426] border border-[#2563EB]/30 rounded-2xl overflow-hidden shadow-[0_0_40px_rgba(37,99,235,0.15)]">
        {/* Top accent */}
        ...
        <div className="relative p-8">
          {/* Badge */}
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#2563EB]/20 border border-[#2563EB]/40 text-[#60A5FA] text-xs font-bold uppercase tracking-wider mb-5">
            <BookOpen size={10} />
            Guía gratuita
          </span>
          {/* Title */}
          <h4 className="font-syne font-bold text-white text-xl leading-tight mb-2">
            Roadmap del Trading Algorítmico
          </h4>
          <p className="text-[#60A5FA] text-sm font-medium mb-6">
          ...
          <ul className="space-y-3">
            {GUIDE_STEPS.map((step, i) => (
              <li key={i} className="flex items-start gap-3">
                <span className="w-5 h-5 rounded-md bg-[#2563EB]/20 border border-[#2563EB]/30 flex items-center justify-center text-[#60A5FA] text-[10px] font-bold font-mono flex-shrink-0 mt-0.5">
                  {i + 1}
                </span>
                <span className="text-[#94A3B8] text-xs leading-snug">{step}</span>
              </li>
            ))}
          </ul>
          {/* Footer */}
          <div className="mt-6 pt-5 border-t border-[#2563EB]/15 flex items-center justify-between">
            <span className="text-[#4A5568] text-xs font-mono">{BRAND.name}</span>
            <span className="text-[#4A5568] text-xs font-mono">PDF · Gratuito</span>
          </div>
        </div>
      </div>
    </div>
...
export default function LeadMagnet() {
  ...
  return (
    <section
      id="guia-gratuita"
      className="py-20 md:py-28 bg-[#050D1C] relative overflow-hidden"
    >
      {/* Background glow */}
      <div
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-14 items-center">
          ...
          <div>
            ...
            <p className="text-[#94A3B8] leading-relaxed mb-7">
            ...
            {submitted ? (
              <div className="bg-emerald-500/10 border border-emerald-500/25 rounded-xl p-5 flex items-center gap-3">
                ...
                <div>
                  <p className="text-white font-medium text-sm">¡Perfecto, la guía está en camino!</p>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      required
                      className="w-full pl-10 pr-4 py-3 bg-[#0A1628] border border-[#2563EB]/25 text-[#E2E8F0] placeholder-[#4A5568] rounded-xl text-sm focus:outline-none focus:border-[#2563EB]/60 focus:shadow-[0_0_0_2px_rgba(37,99,235,0.15)] transition-all duration-200"
                    />
                  </div>
                  <button
                    type="submit"
